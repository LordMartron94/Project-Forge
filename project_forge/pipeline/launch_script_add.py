...
class AddLaunchScript(IPipe):
    ...
    def flow(self, data: PipelineContext) -> PipelineContext:
        ...
        launch_script_path: Path = data.repo_path.joinpath("launch.ps1")
        with open(launch_script_path, "w") as launch_script:
            launch_script.write(f"""./venv/Scripts/activate.ps1
    
# Get the current working directory
$originalDir = Get-Location
$newDir = Join-Path -Path $originalDir -ChildPath "{data.project_root_name_sanitized}" -AdditionalChildPath ("components", "MD.Launcher")

# Add the project's root directory to PYTHONPATH
$env:PYTHONPATH = "$newDir" + ";" + $env:PYTHONPATH

py "./{data.repo_path.joinpath(data.project_root_name_sanitized, "components", "MD.Launcher", "md_launcher", "components", "launcher", "launch.py").relative_to(data.repo_path)}" "./launch_config.json"
""")
