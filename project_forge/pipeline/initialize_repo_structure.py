...
class InitializeRepoStructure(IPipe):
    ...
    def flow(self, data: PipelineContext) -> PipelineContext:
        ...
        data.repo_path.joinpath("build").mkdir(parents=True, exist_ok=True)

        data.project_path.mkdir(parents=True, exist_ok=True)

        folders_to_create = ["components", "tests", "benchmarks", "docs"]

        for folder in folders_to_create:
            data.project_path.joinpath(folder).mkdir(parents=True, exist_ok=True)

        if data.multi_language:
            with open(data.project_path.joinpath("requirements.txt"), "w") as requirements_file:
                requirements_file.write(f"""
-r components/MD.Logging/requirements.txt
				""")
            with open(data.repo_path.joinpath("launch_config.json"), "w") as launch_config_file:
            ...
            with open(data.repo_path.joinpath("todo.txt"), "w") as todo_file:
